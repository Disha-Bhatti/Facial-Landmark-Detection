...
predictor = dlib.shape_predictor("D:\Teksun\\face-detection\shape_predictor_68_face_landmarks.dat")
...
cv2.imwrite('D:\Teksun\\face-detection\cropped_pic\\final image.jpg', img_resize)
...
while True:
    if webcam:
        ...
        img = cv2.imread('C:\\Users\Disha\Desktop\\faceDetection\\test1.jpg')
    img = cv2.resize(img, (0, 0), None, 1, 1)
    imgOriginal = img.copy()
    ...
    for face in faces:
        ...
        def createBox(img, points, scale=5, masked= False,cropped= True):
            if masked:
                mask = np.zeros_like(img)    #np.ones((5,5),np.uint8)
                mask = cv2.fillPoly(mask, [points], (255, 255, 255))
                img = cv2.bitwise_and(img, mask)
                #cv2.imshow('Mask',mask)
            if cropped:
